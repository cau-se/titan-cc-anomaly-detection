...
/**
 * Contains a web server for accessing the power anomalies via a REST interface. This API is modeled
 * after the API from the History-Microservice to allow easier adaptation by the frontend. A trend
 * does not make that much sense for anomalies, so that route was left out.
 */
public class RestApiServer {
  ...
  public RestApiServer(final Session cassandraSession, final String tableNameSuffix,
    ...
    this.aggregatedRepository =
        ActivePowerRepository.forAggregated(cassandraSession, tableNameSuffix);
  ...
  public void start() { // NOPMD declaration of routes
    ...
    this.webService.get("/aggregated-power-anomaly/:identifier/distribution",
        (request, response) -> {
          final String identifier = request.params("identifier");
          final long after = NumberUtils.toLong(request.queryParams("after"), 0);
