...
public class RestApiClient {
   ...
   * 
   * @param statsBaseUrl Base URL for the Stats-API
   * @param configBaseUrl Base URL for the Config-API
   */
   ...
   * @return A {@link CompletableFuture} that is completed when the server has responded.
   */
  public CompletableFuture<List<DayOfWeekActivePowerRecord>> getDayOfWeek(final String sensor) {
    final HttpRequest request = HttpRequest.newBuilder()
        .GET()
        .uri(URI.create(this.statsBaseUrl + SENSOR_STATS_URL_PART + sensor + "/day-of-week"))
        .build();
    return this.client.sendAsync(request, HttpResponse.BodyHandlers.ofString())
        .thenApply(HttpResponse::body)
        .thenApply(json -> this.gson.fromJson(json, this.dayOfWeekActivePowerRecordListType));
  }
  ...
  public CompletableFuture<List<HourOfDayActivePowerRecord>> getHourOfDay(final String sensor) {
    final HttpRequest request = HttpRequest.newBuilder()
        .GET()
        .uri(URI.create(this.statsBaseUrl + SENSOR_STATS_URL_PART + sensor + "/hour-of-day"))
        .build();
    return this.client.sendAsync(request, HttpResponse.BodyHandlers.ofString())
        .thenApply(HttpResponse::body)
        .thenApply(json -> this.gson.fromJson(json, this.hourOfDayActivePowerRecordListType));
  }
  ...
  public CompletableFuture<List<HourOfWeekActivePowerRecord>> getHourOfWeek(final String sensor) {
    final HttpRequest request = HttpRequest.newBuilder()
        .GET()
        .uri(URI.create(this.statsBaseUrl + SENSOR_STATS_URL_PART + sensor + "/hour-of-week"))
        .build();
    return this.client.sendAsync(request, HttpResponse.BodyHandlers.ofString())
        .thenApply(HttpResponse::body)
        .thenApply(json -> this.gson.fromJson(json, this.hourOfWeekActivePowerRecordListType));
  }
  ...
  public CompletableFuture<List<String>> getSensorIdentifiers() {
    final HttpRequest request = HttpRequest.newBuilder()
        .GET()
        .uri(URI.create(this.configBaseUrl + "/sensor-registry"))
        .build();
    return this.client.sendAsync(request, HttpResponse.BodyHandlers.ofString())
        .thenApply(HttpResponse::body)
