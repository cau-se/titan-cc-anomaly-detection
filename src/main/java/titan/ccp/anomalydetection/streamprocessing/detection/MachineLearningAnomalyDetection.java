...
/**
 * Dummy anomaly detection implementation with a machine learning model. This implementation does
 * not work but shows how the machine learning forecast might be integrated into this anomaly
 * detection microservice. This class is abstract so that nobody thinks of initializing it.
 */
...
public abstract class MachineLearningAnomalyDetection implements AnomalyDetection {
  ...
  public boolean activePowerRecordAnomalyDetection(final ActivePowerRecord record) {
    return this.detectAnomaly(record.getIdentifier(), record.getTimestamp(), record.getValueInW());
  }

  @Override
  public boolean aggregatedActivePowerRecordAnomalyDetection(
      final AggregatedActivePowerRecord record) {
    return this.detectAnomaly(record.getIdentifier(), record.getTimestamp(), record.getSumInW());
  }
