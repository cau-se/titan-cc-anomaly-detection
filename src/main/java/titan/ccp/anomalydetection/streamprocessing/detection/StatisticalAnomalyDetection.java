 ...
 * this implementation detects values that are further than three standard deviations away from the
 * mean of every hour of a week.
 */
public class StatisticalAnomalyDetection implements AnomalyDetection {
   ...
   * 
   * @param timeZone The timezone that should be used for converting timestamps into local date-time
   * @param numStandardDeviations The number of standard deviations that a record has to be away
   *        from the mean to be classified as an outlier
   */
  ...
  public boolean activePowerRecordAnomalyDetection(final ActivePowerRecord record) {
    return this.detectAnomaly(record.getIdentifier(), record.getTimestamp(),
        record.getValueInW(), this.tolerance);
  }

  @Override
  public boolean aggregatedActivePowerRecordAnomalyDetection(
      final AggregatedActivePowerRecord record) {
    return this.detectAnomaly(record.getIdentifier(), record.getTimestamp(),
        record.getSumInW(), this.aggregatedTolerance);
  }
  ...
  private boolean detectAnomaly(final String identifier, final long timestamp,
    ...
    List<HourOfWeekActivePowerRecord> stats = this.statisticsCache.getHourOfWeek().get(identifier);
    if (stats == null) {
      return false;
    }
    stats = stats.stream()
        .filter(elem -> elem.getDayOfWeek() == dayOfWeek && elem.getHourOfDay() == hour)
    ...
    final boolean anomaly =
        this.isOutlier(value, prediction.getMean(), prediction.getPopulationVariance(), tolerance);
  ...
  private boolean isOutlier(final double value, final double mean, final double variance,
    ...
    final boolean outlier =
        Math.abs(meanRounded - valueRounded) > this.numStandardDeviations * standardDeviation
            + tolerance;
    if (LOGGER.isInfoEnabled() && outlier) {
      ...
      LOGGER.info("  {}x Standard Deviation: {}", this.numStandardDeviations,
          this.numStandardDeviations * standardDeviation);
