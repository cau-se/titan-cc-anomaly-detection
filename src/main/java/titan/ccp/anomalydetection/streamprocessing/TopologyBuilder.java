...
public class TopologyBuilder {
  ...
  public Topology build() {
    ...
    this.createTableIfNotExists(normalCassandraWriter,
        new ActivePowerRecord("", 0, 0));
    this.createTableIfNotExists(aggregatedCassandraWriter,
        new AggregatedActivePowerRecord("", 0, 0, 0, 0, 0, 0));
    this.builder
        .stream(this.inputTopic, Consumed.with(
            Serdes.String(),
            IMonitoringRecordSerde.serde(new ActivePowerRecordFactory())))
        .filter((key, record) -> this.anomalyDetection.activePowerRecordAnomalyDetection(record))
        .peek((key, record) -> LOGGER.info(
            "Anomaly detected! Writing ActivePowerRecord {} to Cassandra\n",
    ...
    this.builder
        ...
        .filter((key, record) -> this.anomalyDetection
            .aggregatedActivePowerRecordAnomalyDetection(record))
        .peek((key, record) -> LOGGER.info(
            "Anomaly detected! Writing AggregatedActivePowerRecord {} to Cassandra\n",
  ...
  private CassandraWriter<IMonitoringRecord> buildCassandraWriter(
    ...
    primaryKeySelectionStrategy.registerPartitionKeys(
        recordClass.getSimpleName() + this.tableNameSuffix, "identifier");
    primaryKeySelectionStrategy.registerClusteringColumns(
        recordClass.getSimpleName() + this.tableNameSuffix, "timestamp");

    return CassandraWriter.builder(this.cassandraSession, new KiekerDataAdapter())
        .tableNameMapper(
            r -> PredefinedTableNameMappers.SIMPLE_CLASS_NAME.apply(r) + this.tableNameSuffix)
        .primaryKeySelectionStrategy(primaryKeySelectionStrategy)
        .build();
  }

  /*
   * If the tables don't exist, the API will throw an error when trying to access them. This is a
   * really inappropriate way to handle this but the CassandraWriter does not create tables when
   * started but only when written to. And the createTableIfNotExists method is private but I can't
   * change the titan-ccp-commons library.
   */
  private void createTableIfNotExists(final CassandraWriter<IMonitoringRecord> cassandraWriter,
    ...
    try {
      ...
      createTableIfNotExists.invoke(cassandraWriter,
          record.getClass().getSimpleName() + this.tableNameSuffix, record);
